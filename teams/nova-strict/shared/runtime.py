...
from .commands import (AgentDecision, ModelDecision, fallback_decision, forced_decision,
                       idle_wire, to_wire, validate_model_decision)
from .perception import allowed_commands, build_perception, validate_semantics
...
# Half of all decisions offer the model a mask of exactly one command. Set to 0 to
# spend the model call anyway and compare decision quality.
SKIP_SINGLETON_MASK = os.environ.get("AFC_SKIP_SINGLETON_MASK", "1") != "0"
...
def invoke_agent(agent: Any, player_id: int, payload: dict) -> InvocationResult:
    ...
    if SKIP_SINGLETON_MASK:
        mask = allowed_commands(perception)
        if len(mask) == 1:
            forced = _apply(forced_decision(perception, mask[0]), perception, team_id, player_id)
            if forced is not None:
                return InvocationResult(forced, "mask-forced",
                                        f"Единственная доступная команда {mask[0]}.",
    ...
    for attempt in range(2):
        ...
        try:
            ...
            decision = validate_model_decision(model_decision, perception)
            ...
            error = caught
        break
    # The baseline idled here, which cost the ball owner a whole 2 s tick.
    # A deterministic command derived from the same observation is always better.
    decision = fallback_decision(perception)
    wire = _apply(decision, perception, team_id, player_id)
    if wire is None:
        return InvocationResult(idle_wire(team_id, player_id), "error-idle",
                                "Ни модель, ни детерминированный резерв не дали команду.",
                                round((time.perf_counter()-started)*1000),
                                f"{type(error).__name__}: {error}", final_prompt)
    return InvocationResult(wire, "deterministic-fallback", decision.rationale,
                            round((time.perf_counter()-started)*1000),
                            f"{type(error).__name__}: {error}", final_prompt)


def _apply(decision: AgentDecision, perception, team_id: int, player_id: int) -> dict | None:
    """Run the same semantic check the model output goes through, then serialise."""
    command = decision.command.model_dump()
    try:
        validate_semantics(perception, command["command_type"], command.get("target_player_id"),
                           command.get("target_x"), command.get("target_y"))
    except (ValidationError, ValueError):
        return None
    return to_wire(decision, team_id, player_id)
