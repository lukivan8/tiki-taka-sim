...
# Three strict rules this team is built around: shoot whenever both true (in the
# attacking two-thirds and a shot is geometrically live), run the ball straight
# ahead when no opponent stands in the forward corridor, never target a point
# closer than MIN_TEAMMATE_SEPARATION to a teammate.
ATTACK_TWO_THIRDS_X = -FIELD_X / 3
FORWARD_CORRIDOR_LENGTH, FORWARD_CORRIDOR_HALF_WIDTH = 15.0, 6.0
MIN_TEAMMATE_SEPARATION = 3.0
...
def _avoid_collisions(point: "Point", perception: "Perception") -> "Point":
    """Push a target away from any teammate closer than MIN_TEAMMATE_SEPARATION.

    Anchors and forward-run targets are computed independently per player, so
    without this two role corridors that briefly overlap could both compute a
    point next to each other. This keeps every computed target a clean slot.
    """
    px, py = point.x, point.y
    for pid, mate in perception.teammates.items():
        if pid == perception.player_id:
            continue
        dx, dy = px-mate.position.x, py-mate.position.y
        distance = math.hypot(dx, dy)
        if distance < MIN_TEAMMATE_SEPARATION:
            if distance < 0.05:
                dx, dy, distance = 1.0, 0.0, 1.0
            scale = (MIN_TEAMMATE_SEPARATION-distance)/distance
            px, py = px+dx*scale, py+dy*scale
    return Point(_clamp(px, -FIELD_X, FIELD_X), _clamp(py, -FIELD_Y, FIELD_Y))
...
def dynamic_anchor(perception: Perception, player_id: int) -> Point:
    ...
    if player_id == 0:
        point = Point(-50, _clamp(ball.y*0.10, -3.2, 3.2))
    elif player_id == 1:
        point = Point(_clamp(ball.x-13, -38, -18), _clamp(ball.y*0.28, -10, 10))
    elif player_id == 2:
        offset = -10 if ours else -14 if theirs else -11
        point = Point(_clamp(ball.x+offset, -20, 30), _clamp(ball.y*0.38, -12, 12))
    else:
        ...
        point = Point(_clamp(ball.x+offset, -8, 46),
                      _clamp(side*14+ball.y*0.22, -24 if side < 0 else 8,
                             -8 if side < 0 else 24))
    return _avoid_collisions(point, perception)
...
def _forward_clear(perception: Perception) -> bool:
    """True if no opponent stands in the corridor straight ahead of the carrier."""
    origin = perception.self_player.position
    for opponent in perception.opponents.values():
        ahead = opponent.position.x - origin.x
        lateral = abs(opponent.position.y - origin.y)
        if 0 < ahead <= FORWARD_CORRIDOR_LENGTH and lateral <= FORWARD_CORRIDOR_HALF_WIDTH:
            return False
    return True
...
def allowed_commands(perception: Perception) -> tuple[str, ...]:
    ...
    if perception.owns_ball:
        if perception.player_id == 0:
            ...
            shot_ready = shot_distance <= SHOT_RANGE and bool(visible)
            # Rule 1: a live shot in the attacking two-thirds is never optional.
            if shot_ready and perception.self_player.position.x >= ATTACK_TWO_THIRDS_X:
                return ("SHOOT",)
            # Rule 2: nobody ahead of you with the ball means you run, not pass sideways.
            if _forward_clear(perception) and perception.self_player.position.x < FIELD_X-4:
                return ("DRIBBLE",)
        ...
        if perception.possession_team is None:
            # Rule 4: chasing a loose ball is always INTERCEPT, never MOVE_TO.
            # MOVE_TO freezes its target at decision time; the ball keeps rolling
            # for the whole tick, so a frozen target makes the runner arrive
            # where the ball used to be and peel off short. INTERCEPT recomputes
            # its aim point (ball position + velocity * lookahead) every physics
            # frame, so the chase always tracks the ball's live position.
            return ("INTERCEPT",)
...
def validate_semantics(perception: Perception, command_type: str,
    ...
    if command_type in {"MOVE_TO", "DRIBBLE"}:
        # Rule 3: never aim at a point a teammate already occupies.
        target = Point(float(target_x), float(target_y))
        nearest_mate = min(
            (mate.position.distance_to(target) for pid, mate in perception.teammates.items()
             if pid != perception.player_id), default=99.0)
        if nearest_mate < MIN_TEAMMATE_SEPARATION:
            raise ValueError(
                f"{command_type} цель ближе {MIN_TEAMMATE_SEPARATION:.1f} м к партнёру — риск столкновения")
