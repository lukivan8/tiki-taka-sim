...
from .perception import (build_perception, dynamic_anchor, executable_dribble_corridors,
                         open_passes, validate_semantics)
            ...
            if attempt == 0:
                ...
                if isinstance(caught, ValueError) and "MOVE_TO" in str(caught):
                    ...
                    repair = (f" Безопасное исправление: MOVE_TO к динамической точке "
                              f"({anchor.x:.1f},{anchor.y:.1f}) со sprint=false.")
                elif perception.owns_ball:
                    passes = open_passes(perception)
