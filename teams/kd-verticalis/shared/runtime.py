...
from .perception import Point, build_perception, sprint_directive, validate_semantics
...
def _force_sprint(wire: dict, perception) -> None:
    """Overwrite the model's pace flag with the computed one.

    Left to itself the model picked `sprint: false` on 52 of 52 MOVE_TO
    commands in a real match, capping the whole team at walkSpeed. Pace is a
    physics question the perception layer already answers, so it is applied
    rather than requested: rejecting the command instead would burn the single
    retry and risk falling through to IDLE, which is slower still.
    """
    parameters = wire.get("parameters") or {}
    kind = wire.get("commandType")
    if kind not in {"MOVE_TO", "DRIBBLE", "INTERCEPT", "SLIDE_TACKLE", "PRESS_BALL"}:
        return
    target = None
    if kind in {"MOVE_TO", "DRIBBLE"} and {"target_x", "target_y"} <= parameters.keys():
        sign = 1 if int(perception.team_id) == 0 else -1
        target = Point(sign*float(parameters["target_x"]), sign*float(parameters["target_y"]))
    sprint, _ = sprint_directive(perception, target)
    if kind == "INTERCEPT":
        parameters["aggressive"] = sprint
    elif kind == "PRESS_BALL":
        # Intensity above sprintPressThreshold (0.65) is what makes a press run.
        if sprint:
            parameters["intensity"] = max(float(parameters.get("intensity", 0.0)), 0.9)
    else:
