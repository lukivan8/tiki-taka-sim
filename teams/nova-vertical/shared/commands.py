...
from .perception import (Perception, Point, allowed_commands, best_pass_target,
                         best_shot_aim, dynamic_anchor)
...
def _forward_point(perception: Perception) -> Point:
    ...
    if anchor.x >= origin.x+2.5 and origin.distance_to(anchor) >= 2.5:
        return anchor
    return Point(min(origin.x+6.0, 46.0), max(-30.0, min(30.0, origin.y*0.8)))
