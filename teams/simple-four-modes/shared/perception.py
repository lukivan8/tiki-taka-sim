...
class FreeBallRace:
    ...
    def margin(self) -> float:
        return self.opponent_time-self.our_time

    @property
    def should_intercept(self) -> bool:
        # A reachable positive edge is enough. Narrow advantages are exactly
        # the loose-ball windows an already-committed forward can win.
        return self.reachable and self.margin > 0
...
def _intercept_estimate(player: Player, perception: Perception) -> InterceptEstimate:
    for step in range(13):
        seconds = step*0.25
        point = perception.ball.projected(perception.ball_velocity, seconds)
        if _arrival_time(player, point) <= seconds+0.12:
            return InterceptEstimate(seconds, point, True, seconds)
    point = perception.ball.projected(perception.ball_velocity, 3.0)
    return InterceptEstimate(_arrival_time(player, point), point, False, 3.0)


def _intercept_time(player: Player, perception: Perception) -> tuple[float, Point]:
    estimate = _intercept_estimate(player, perception)
    return estimate.arrival_time, estimate.point
...
def free_ball_race(perception: Perception) -> FreeBallRace:
    """Whether our selected responder can actually win a free ball."""
    team_rank, enemy_rank = responsibility(perception)
    our_time, player_id, point = team_rank[0]
    estimate = _intercept_estimate(perception.teammates[player_id], perception)
    return FreeBallRace(player_id, point, our_time, enemy_rank[0][0], estimate.reachable)
...
def control_phase(perception: Perception) -> str:
    ...
    race = free_ball_race(perception)
    our_time, their_time = race.our_time, race.opponent_time
...
def dynamic_anchor(perception: Perception, player_id: int) -> Point:
    ...
    # Wingers may anticipate a likely recovery. The only centre-back must wait
    # for secured possession before abandoning rest defence.
    attacking = (phase == "OUR_CONTROL" if player_id == 2
                 else phase in {"OUR_CONTROL", "LIKELY_OURS"})
    if player_id == 0:
        # Stand six metres in front of the goal centre on the line from the
        # ball to that centre. This closes the real shooting angle much more
        # faithfully than a fixed percentage of ball.y, especially close to goal.
        distance_from_goal = max(6.0, ball.x + GOAL_X)
        target_y = ball.y * 6.0 / distance_from_goal
        return Point(-50, _clamp(target_y, -3.2, 3.2))
    if attacking:
        if player_id == 2:
            return Point(_clamp(ball.x-20, -36, 8), _clamp(ball.y*0.10, -5, 5))
        if player_id in {1, 3}:
            run_target = winger_run_target(perception, player_id)
        ...
        anchor = _separated_anchor(
            perception, player_id,
            Point(_clamp(ball.x+12, -4, 47), _clamp(ball.y*0.15, -6, 6)),
        )
        if open_forward_receiver(perception, player_id) and anchor.x < perception.self_player.position.x:
            # Do not make an open striker retreat with the ball. A small
            # forward step also gives THROUGH passes a receiving run without
            # inventing a long speculative sprint.
            candidate = Point(_clamp(perception.self_player.position.x+2, -8, 47), anchor.y)
            owner = perception.teammates[perception.possession_player]
            anchor = (candidate if perception.pass_line(owner.position, candidate).open
                      else Point(perception.self_player.position.x, anchor.y))
        return anchor
...
def winger_run_target(perception: Perception, player_id: int) -> Point | None:
    """Ball-relative pass-and-run target: get level with the attack, not the old anchor."""
    if (player_id not in {1, 3} or control_phase(perception) != "OUR_CONTROL" or
            perception.possession_player == player_id):
        return None
    player = perception.teammates[player_id]
    side = -1 if player_id == 1 else 1
    owner = perception.teammates[perception.possession_player]
    if owner.player_id == 4:
        candidates = [
            Point(_clamp(perception.ball.x+6, -12, 44), side*9),
            Point(_clamp(perception.ball.x+2, -12, 44), side*14),
            Point(_clamp(perception.ball.x-4, -12, 44), side*12),
        ]
    else:
        candidates = [
            Point(_clamp(perception.ball.x+10, -12, 44), side*14),
            Point(_clamp(perception.ball.x+6, -12, 44), side*18),
            Point(_clamp(perception.ball.x+2, -12, 44), side*12),
        ]

    def space(point: Point) -> tuple[float, float]:
        opponent_space = min(enemy.position.distance_to(point)
        ...
        teammate_space = min((mate.position.distance_to(point)
                              ...
                              if pid != player_id), default=99)
        return opponent_space, teammate_space

    usable = [point for point in candidates if min(space(point)) >= 5]
    teammate_safe = [point for point in candidates if space(point)[1] >= 5]
    if usable:
        target = usable[0]
    elif teammate_safe:
        target = max(teammate_safe, key=lambda point: space(point)[0])
    else:
        return None
    if target.x <= player.position.x and perception.ball.x <= player.position.x:
        return None
    return target


def open_forward_receiver(perception: Perception, player_id: int) -> bool:
    """The striker is already ahead of a teammate and directly available."""
    if (player_id != 4 or perception.possession_team != perception.team_id or
            perception.possession_player in {None, player_id}):
        return False
    owner = perception.teammates[perception.possession_player]
    player = perception.teammates[player_id]
    return (player.position.x > owner.position.x and
            perception.pass_line(owner.position, player.position).open)
...
def dribble_corridors(perception: Perception) -> list[tuple[str, Point, float, int]]:
    ...
    if perception.player_id in {1, 3}:
        straight = next(item for item in values if item[0] == "прямо")
        if straight[2] > PANIC_DISTANCE:
            return [straight] + sorted((item for item in values if item[0] != "прямо"),
...
def attacking_open_passes(perception: Perception,
                          player_ids: set[int] | None = None) -> list[tuple[int, PassLine]]:
    """Safe releases that do not move the counterattack back toward our goal."""
    origin_x = perception.self_player.position.x
    return [(pid, line) for pid, line in open_passes(perception, player_ids)
            if perception.teammates[pid].position.x >= origin_x]


def defensive_clearance_emergency(perception: Perception) -> bool:
    """Reserve CLEAR for a trapped outfielder deep in our defensive third."""
    return (perception.owns_ball and perception.player_id != 0 and
            perception.self_player.position.x <= DEFENSIVE_CLEAR_X and
            panic_pressure(perception)[0] and
            not attacking_open_passes(perception) and
            not executable_dribble_corridors(perception))
...
def good_enough_shot(perception: Perception) -> bool:
    """An open shot from the final quarter is worth taking before another carry."""
    if not perception.owns_ball or perception.player_id == 0:
        return False
    _, _, visible, _ = _shot_geometry(perception, perception.self_player.position)
    return perception.self_player.position.x >= FIELD_X/2 and bool(visible)


def panic_pressure(perception: Perception) -> tuple[bool, int, float]:
    """One deliberately simple definition of immediate pressure on the ball owner."""
    nearest = min(perception.opponents.values(),
                  key=lambda enemy: enemy.position.distance_to(perception.self_player.position))
    distance = nearest.position.distance_to(perception.self_player.position)
    return perception.owns_ball and distance <= PANIC_DISTANCE, nearest.player_id, distance


def tactical_mode(perception: Perception) -> str:
    """One exclusive mode, selected in the same order presented to the model."""
    if good_enough_shot(perception):
        return "FINISH"
    if panic_pressure(perception)[0]:
        return "PANIC"
    if perception.owns_ball and perception.player_id == 0:
        return "PROGRESS"
    if (perception.owns_ball and perception.player_id != 0 and
            (attacking_open_passes(perception) or executable_dribble_corridors(perception))):
        return "PROGRESS"
    return "SUPPORT"
...
def allowed_commands(perception: Perception) -> tuple[str, ...]:
    ...
    # Goalkeeper movement without the ball is generated deterministically.
    # Once he owns it, the model may only distribute or clear, never carry it
    # along the goal line via MOVE_TO.
    base = [] if perception.owns_ball and perception.player_id == 0 else ["MOVE_TO"]
    ...
    if perception.owns_ball:
        if perception.player_id == 0:
            ...
            base += ["PASS"]
            if defensive_clearance_emergency(perception):
                base += ["CLEAR"]
        ...
        if (perception.possession_team is None and
                free_ball_race(perception).should_intercept):
            base += ["INTERCEPT"]
        elif phase == "THEIR_CONTROL":
            # INTERCEPT predicts a loose ball; PRESS_BALL follows its owner.
            base += ["PRESS_BALL"]
...
def _describe_legacy(perception: Perception, player_config: dict) -> str:
    phase = control_phase(perception)
    ...
    for pid, mate in sorted(perception.teammates.items()):
        ...
        progression = mate.position.x-origin.x
        direction = "ВПЕРЁД/ПОПЕРЁК" if progression >= 0 else "НАЗАД"
        backward_warning = " (НЕ АТАКУЮЩИЙ ВАРИАНТ)" if progression < 0 else ""
        lines.append(f"- к №{pid}: {'ОТКРЫТА' if line.open else 'РИСК'}-{direction}; "
                     f"{backward_warning}{line.distance:.1f} м/{line.ball_time:.2f} с; "
                     f"перехватчик {line.nearest_interceptor if line.nearest_interceptor is not None else 'нет'}, "
                     f"запас {line.margin:.2f} с; давление {pressure:.1f} м; продвижение {progression:+.1f}; "
    ...
    if perception.owns_ball and perception.player_id != 0:
        in_panic, pressure_id, pressure_distance = panic_pressure(perception)
        if in_panic:
            releases = attacking_open_passes(perception)
            if releases:
                lines.append(f"PANIC ZONE: соперник №{pressure_id} в {pressure_distance:.1f} м. "
                             "Есть OPEN-ВПЕРЁД/ПОПЕРЁК: быстро освободи мяч; DRIBBLE сейчас запрещён.")
            else:
                lines.append(f"PANIC ZONE: соперник №{pressure_id} в {pressure_distance:.1f} м, "
                             "но атакующего OPEN PASS нет. Пас назад не является release; "
                             "используй открытый carry или лучшее доступное действие.")
                carries = executable_dribble_corridors(perception)
                if carries:
                    name, target, clearance, _ = carries[0]
                    lines.append(f"FORWARD CARRY PRIORITY ЭТОГО ТИКА: PASS НАЗАД не выбирай. "
                                 f"Выбери DRIBBLE {name} к ({target.x:.1f},{target.y:.1f}) "
                                 f"sprint=true; свободный радиус {clearance:.1f} м.")
    ...
    if perception.player_id in {1, 3} and not perception.owns_ball:
        run_target = winger_run_target(perception, perception.player_id)
        if run_target is not None:
            owner = perception.teammates[perception.possession_player]
            lines.append(f"WINGER PASS AND RUN: владелец №{owner.player_id} в x={owner.position.x:.1f}; "
                         "не оставайся в глубокой линии. "
                         f"Атакуй ({run_target.x:.1f},{run_target.y:.1f}) через MOVE_TO sprint=true, "
                         "чтобы стать вертикальной или диагональной целью передачи.")
    ...
    lines.append("Ролевые зоны без мяча:" if perception.owns_ball else "Ролевые зоны:")
    ...
    elif good_enough_shot(perception):
        lines.append("GOOD_ENOUGH_SHOT: ты в последней четверти и минимум одно направление открыто. "
                     "Ударь сейчас; ещё один DRIBBLE запрещён, потому что защита успеет перестроиться.")
...
# The source team keeps the detailed renderer above. This variant intentionally
# overrides it with a compact observation organized around four exclusive modes.
def describe(perception: Perception, player_config: dict) -> str:
    phase = control_phase(perception)
    mode = tactical_mode(perception)
    team_rank, enemy_rank = responsibility(perception)
    primary_time, primary_id, primary_point = team_rank[0]
    race = free_ball_race(perception) if perception.possession_team is None else None
    nearest_enemy = min(perception.opponents.values(),
                        key=lambda enemy: enemy.position.distance_to(perception.self_player.position))
    pressure_distance = nearest_enemy.position.distance_to(perception.self_player.position)
    shot_distance, shot_angle, visible, blockers = _shot_geometry(
        perception, perception.self_player.position)
    if high_quality_shot(perception):
        shot_quality = "HIGH_QUALITY"
    elif good_enough_shot(perception):
        shot_quality = "GOOD_ENOUGH"
    else:
        shot_quality = "NOT_READY"

    if perception.possession_team is None:
        possession = "FREE"
    elif perception.owns_ball:
        possession = "YOU"
    elif perception.possession_team == perception.team_id:
        possession = f"TEAMMATE_#{perception.possession_player}"
    else:
        possession = f"OPPONENT_#{perception.possession_player}"

    lines = [
        f"Время: {perception.game_time:.1f} с. Фаза: {phase}. Владение: {possession}.",
        f"Ты №{perception.player_id}: ({perception.self_player.position.x:.1f},"
        f"{perception.self_player.position.y:.1f}); мяч: ({perception.ball.x:.1f},"
        f"{perception.ball.y:.1f}); до мяча {perception.distance_to_ball():.1f} м.",
        f"ТАКТИЧЕСКИЙ РЕЖИМ: {mode}. Это первый применимый режим; более низкие режимы игнорируй.",
        f"PRESSURE: {'PANIC' if perception.owns_ball and pressure_distance <= PANIC_DISTANCE else 'CALM'}; "
        f"ближайший соперник №{nearest_enemy.player_id} в {pressure_distance:.1f} м.",
    ]
    if perception.player_id == 0:
        lines.append("ВРАТАРЬ: позиционирование без мяча выполняется детерминированно; "
                     "модель сейчас выбирает только доставку мяча.")
    else:
        lines.append(
            f"УДАР: SHOT_QUALITY: {shot_quality}; доступен={'да' if 'SHOOT' in allowed_commands(perception) else 'нет'}; "
            f"дистанция={shot_distance:.1f} м; угол={shot_angle:.1f}°; "
            f"открыто={visible or 'нет'}; blockers={blockers or 'нет'}.")

    if mode == "FINISH":
        aim = "CENTER" if "CENTER" in visible else visible[0]
        lines.append(f"РЕШЕНИЕ FINISH: выбери SHOOT aim_location={aim} power=1.0. "
                     "Не выбирай PASS, DRIBBLE или MOVE_TO.")
    elif mode == "PANIC":
        releases = attacking_open_passes(perception)
        if perception.player_id == 0:
            releases = [(pid, line) for pid, line in releases
                        if perception.teammates[pid].position.distance_to(
                            perception.self_player.position) >= 6]
            if releases:
                lines.append("РЕШЕНИЕ PANIC ВРАТАРЯ: немедленно выбери GK_DISTRIBUTE одному из " + ", ".join(
                    f"№{pid} margin={line.margin:.2f}" for pid, line in releases) +
                    ". Используй THROW до 25 м, иначе KICK. Не выбирай PASS или MOVE_TO.")
            else:
                lines.append("РЕШЕНИЕ PANIC ВРАТАРЯ: безопасной доставки нет; выбери CLEAR. "
                             "Не выбирай PASS или MOVE_TO.")
        elif releases:
            lines.append("РЕШЕНИЕ PANIC: безопасные атакующие передачи: " + ", ".join(
                f"№{pid} margin={line.margin:.2f}" for pid, line in releases) +
                ". Выбери лучший PASS и освободи мяч.")
        else:
            carries = executable_dribble_corridors(perception)
            if carries:
                name, target, clearance, _ = carries[0]
                lines.append(f"РЕШЕНИЕ PANIC: атакующего OPEN PASS нет. Лучшее доступное действие — "
                             f"DRIBBLE {name} к ({target.x:.1f},{target.y:.1f}), "
                             f"свободно {clearance:.1f} м.")
            elif defensive_clearance_emergency(perception):
                lines.append("РЕШЕНИЕ PANIC: ты зажат глубоко у своих ворот, атакующего паса "
                             "и открытого carry нет; выбери CLEAR.")
            else:
                retention_passes = open_passes(perception)
                if retention_passes:
                    lines.append("РЕШЕНИЕ PANIC: атакующего паса и открытого carry нет. "
                                 "Сохрани владение через лучший доступный PASS: " + ", ".join(
                                     f"№{pid} margin={line.margin:.2f}"
                                     for pid, line in retention_passes) +
                                 ". Не выбивай мяч вперёд наугад.")
                else:
                    lines.append("РЕШЕНИЕ PANIC: безопасного паса и открытого carry нет, но это "
                                 "не авария у своих ворот. Выбери наиболее безопасную доступную "
                                 "команду; не выбивай мяч вперёд наугад.")
    elif mode == "PROGRESS":
        passes = sorted(attacking_open_passes(perception),
                        key=lambda item: perception.teammates[item[0]].position.x,
                        reverse=True)
        carries = executable_dribble_corridors(perception)
        if perception.player_id == 0:
            passes = [(pid, line) for pid, line in passes
                      if perception.teammates[pid].position.distance_to(
                          perception.self_player.position) >= 6]
            if passes:
                lines.append("РЕШЕНИЕ PROGRESS ВРАТАРЯ: спокойно и сразу выбери GK_DISTRIBUTE: " + ", ".join(
                    f"№{pid} расстояние={line.distance:.1f} м margin={line.margin:.2f}"
                    for pid, line in passes) +
                    ". Используй THROW до 25 м, иначе KICK. Не выбирай PASS или MOVE_TO.")
            else:
                lines.append("РЕШЕНИЕ PROGRESS ВРАТАРЯ: открытого адресата нет; выбери CLEAR. "
                             "Не выбирай PASS или MOVE_TO.")
        elif passes:
            lines.append("PROGRESS PASS: " + ", ".join(
                f"№{pid} +{perception.teammates[pid].position.x-perception.self_player.position.x:.1f} м "
                f"margin={line.margin:.2f}" for pid, line in passes) + ".")
        if perception.player_id != 0 and carries:
            lines.append("PROGRESS DRIBBLE: " + "; ".join(
                f"{name} -> ({target.x:.1f},{target.y:.1f}), свободно {clearance:.1f} м"
                for name, target, clearance, _ in carries) + ".")
        if perception.player_id == 0:
            pass
        elif passes:
            lines.append("РЕШЕНИЕ PROGRESS: выбери лучший указанный PROGRESS PASS или PROGRESS DRIBBLE. "
                         "PASS с пометкой НАЗАД не является кандидатом этого режима.")
        else:
            lines.append("РЕШЕНИЕ PROGRESS: открытого паса ВПЕРЁД/ПОПЕРЁК нет. Выбери указанный "
                         "PROGRESS DRIBBLE; не выбирай PASS с пометкой НАЗАД.")
    else:
        run_target = winger_run_target(perception, perception.player_id)
        anchor = dynamic_anchor(perception, perception.player_id)
        if run_target is not None:
            lines.append(f"РЕШЕНИЕ SUPPORT: MOVE_TO ({run_target.x:.1f},{run_target.y:.1f}) "
                         "sprint=true — атакуй пространство вперёд и внутрь.")
        elif open_forward_receiver(perception, perception.player_id):
            sprint = anchor.x > perception.self_player.position.x+1
            lines.append(f"РЕШЕНИЕ SUPPORT: ты открытый центральный адресат впереди владельца. "
                         f"Не отступай за мячом; MOVE_TO ({anchor.x:.1f},{anchor.y:.1f}) "
                         f"sprint={'true' if sprint else 'false'} и сохрани глубину для передачи.")
        elif (perception.player_id == primary_id and race is not None and
              race.should_intercept):
            lines.append(f"РЕШЕНИЕ SUPPORT: точка свободного мяча достижима, гонка выиграна "
                         f"с запасом {race.margin:.2f} с; INTERCEPT к "
                         f"({primary_point.x:.1f},{primary_point.y:.1f}).")
        elif perception.possession_team is None:
            if race is not None and race.should_intercept:
                lines.append(f"РЕШЕНИЕ SUPPORT: команда выигрывает свободный мяч с запасом "
                             f"{race.margin:.2f} с, но первичным назначен партнёр №{race.player_id}. "
                             f"Не дублируй его; MOVE_TO ролевую точку "
                             f"({anchor.x:.1f},{anchor.y:.1f}) sprint=false.")
            else:
                reason = ("точка недостижима в горизонте прогноза"
                          if race is not None and not race.reachable
                          else f"нет требуемого преимущества в гонке (margin={race.margin:+.2f} с)")
                lines.append(f"РЕШЕНИЕ SUPPORT: не иди к свободному мячу — {reason}. "
                             f"INTERCEPT запрещён; MOVE_TO ролевую точку "
                             f"({anchor.x:.1f},{anchor.y:.1f}) sprint=false.")
        elif (perception.player_id == primary_id and
              perception.possession_team is not None and
              perception.possession_team != perception.team_id):
            if "SLIDE_TACKLE" in allowed_commands(perception):
                lines.append(f"РЕШЕНИЕ SUPPORT — TACKLE_WINDOW: владелец №{perception.possession_player} "
                             "уже в непосредственной дистанции. Выбери SLIDE_TACKLE "
                             f"target_player_id={perception.possession_player} sprint=true distance=2.15. "
                             "Не выбирай PRESS_BALL: он нужен для сокращения дистанции, а не вместо "
                             "доступного быстрого отбора.")
            else:
                lines.append("РЕШЕНИЕ SUPPORT: сократи дистанцию до владельца командой PRESS_BALL "
                             "intensity=1.0 и удерживай плотный контакт для standing challenge. "
                             "Для PRESS_BALL не указывай target_x, target_y или target_player_id. "
                             "INTERCEPT предназначен только для свободного мяча.")
        else:
            lines.append(f"РЕШЕНИЕ SUPPORT: не иди вторым к мячу. MOVE_TO ролевую точку "
                         f"({anchor.x:.1f},{anchor.y:.1f}) sprint=false.")

    owner = (perception.teammates.get(perception.possession_player)
             if perception.possession_team == perception.team_id else None)
    if perception.owns_ball:
        lines.append("Пасы от тебя:")
        for pid, mate in sorted(perception.teammates.items()):
            if pid == perception.player_id:
                continue
            line = perception.pass_line(perception.self_player.position, mate.position)
            progression = mate.position.x-perception.self_player.position.x
            direction = "ВПЕРЁД/ПОПЕРЁК" if progression >= 0 else "НАЗАД"
            lines.append(f"- №{pid}: {'OPEN' if line.open else 'RISK'}-{direction}; "
                         f"продвижение {progression:+.1f} м; margin {line.margin:.2f}.")
    elif owner is not None:
        line = perception.pass_line(owner.position, perception.self_player.position)
        lines.append(f"Линия владельца к тебе: {'OPEN' if line.open else 'RISK'}, margin={line.margin:.2f}.")

    lines.append("Позиции партнёров: " + "; ".join(
        f"№{pid}=({mate.position.x:.1f},{mate.position.y:.1f})"
        for pid, mate in sorted(perception.teammates.items()) if pid != perception.player_id) + ".")
    lines.append("Позиции соперников: " + "; ".join(
        f"№{pid}=({enemy.position.x:.1f},{enemy.position.y:.1f})"
        for pid, enemy in sorted(perception.opponents.items())) + ".")
    lines.append(f"Ответственность за мяч: первичный №{primary_id} ({primary_time:.2f} с); "
                 f"ближайший соперник к мячу №{enemy_rank[0][1]} ({enemy_rank[0][0]:.2f} с).")
    lines.append("ДОСТУПНЫЕ КОМАНДЫ: " + ", ".join(allowed_commands(perception)) + ".")
    lines.append("Выбери ровно одну команду режима и заполни все её параметры и rationale.")
    return "\n".join(lines)
