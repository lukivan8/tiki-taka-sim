...
from .perception import (allowed_commands, build_perception, dynamic_anchor,
                         executable_dribble_corridors, good_enough_shot,
                         high_quality_shot, open_passes, validate_semantics,
                         winger_run_target)
...
def invoke_agent(agent: Any, player_id: int, payload: dict) -> InvocationResult:
    ...
    if player_id == 0 and not perception.owns_ball:
        anchor = dynamic_anchor(perception, player_id)
        target_x, target_y = anchor.x, anchor.y
        if team_id == 1:
            target_x, target_y = -target_x, -target_y
        wire = {
            "commandType": "MOVE_TO", "teamId": team_id, "playerId": player_id,
            "parameters": {"target_x": target_x, "target_y": target_y, "sprint": False},
            "duration": 0,
        }
        rationale = (f"Детерминированная позиция вратаря по линии мяча: "
                     f"({anchor.x:.1f},{anchor.y:.1f}).")
        return InvocationResult(wire, "deterministic-goalkeeper", rationale, 0)
    ...
    for attempt in range(2):
        ...
        try:
            ...
            if attempt == 0:
                repair = ""
                if perception.owns_ball and player_id == 0:
                    distributions = [(candidate_id, line) for candidate_id, line in open_passes(perception)
                                     if perception.teammates[candidate_id].position.distance_to(
                                         perception.self_player.position) >= 6]
                    if distributions:
                        target_id, line = distributions[0]
                        method = "THROW" if line.distance <= 25 else "KICK"
                        repair = (f" Исправление вратаря: выбери GK_DISTRIBUTE к №{target_id}, "
                                  f"method={method}; не выбирай PASS или MOVE_TO.")
                    else:
                        repair = " Исправление вратаря: выбери CLEAR."
                elif isinstance(caught, ValueError) and "MOVE_TO" in str(caught):
                    run_target = winger_run_target(perception, player_id)
                    anchor = run_target or dynamic_anchor(perception, player_id)
                    repair = (f" Безопасное исправление: MOVE_TO к динамической точке "
                              f"({anchor.x:.1f},{anchor.y:.1f}) со sprint={'true' if run_target else 'false'}.")
                elif high_quality_shot(perception) or good_enough_shot(perception):
                    repair = (" Исправление: выбери SHOOT, укажи открытое aim_location и power=1.0; "
                              "не повторяй DRIBBLE.")
                elif perception.owns_ball:
                    passes = []
                    for candidate_id, line in open_passes(perception):
                        try:
                            validate_semantics(perception, "PASS", target_player_id=candidate_id)
                            passes.append((candidate_id, line))
                        except ValueError:
                            continue
                    dribbles = executable_dribble_corridors(perception)
                    if passes:
                        target_id = passes[0][0]
                        repair = (f" Безопасное исправление: выбери PASS к №{target_id} и обязательно "
                                  "заполни target_player_id и type=GROUND; не повторяй отклонённую цель.")
                    elif dribbles and "DRIBBLE" in allowed_commands(perception):
                        _, target, _, _ = dribbles[0]
                        repair = (f" Безопасное исправление: выбери DRIBBLE к "
                                  f"({target.x:.1f},{target.y:.1f}) и обязательно sprint=false; "
                                  "не повторяй отклонённую цель.")
